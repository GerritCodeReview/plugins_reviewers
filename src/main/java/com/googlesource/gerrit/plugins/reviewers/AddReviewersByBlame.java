...
class AddReviewersByBlame extends AddReviewers {
  ...
  interface Factory {
    AddReviewersByBlame create(ChangeInfo change);
  }
  ...
  AddReviewersByBlame(
    ...
    this.project = new Project.NameKey(change.project);

    ReviewersConfig.ForProject cfg = config.forProject(project);
    this.maxReviewers = cfg.maxReviewers();
    this.ignoreFiles = cfg.ignoreFiles();
  }
  ...
  Set<Account.Id> getReviewers() {
    try (Repository repo = repoManager.openRepository(project);
      ...
      ChangeData cd =
          changeDataFactory.create(reviewDb, project, new Change.Id(changeInfo._number));
      ...
      try {
        ...
        log.error("Couldn't load patchlist for change {}", changeInfo.changeId, ex);
        return ImmutableSet.of();
      }
      ...
      for (PatchListEntry entry : patchList.getPatches()) {
        ...
        if ((entry.getChangeType() == ChangeType.MODIFIED
            ...
            && (ignoreFiles.isEmpty() || !entry.getNewName().matches(ignoreFiles))
      ...
      log.error(
          "Couldn't open project {} for change {}", project.get(), changeInfo.changeId, e);
    }
    ...
    for (Edit edit : edits) {
      for (int i = edit.getBeginA(); i < edit.getEndA(); i++) {
        ...
        for (Account.Id id : ids) {
          ...
          if (account.isActive() && !changeInfo.owner._accountId.equals(account.getId().get())) {
