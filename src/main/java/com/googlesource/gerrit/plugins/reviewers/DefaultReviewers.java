...
class DefaultReviewers implements Runnable {
  ...
  interface Factory {
    DefaultReviewers create(Change change, Set<Account> reviewers, Set<ReviewerFilterSection> filters);
  }

  @Inject
  DefaultReviewers(GerritApi gApi, @Assisted Change change,
      @Assisted Set<Account> reviewers, @Assisted Set<ReviewerFilterSection> filters, @PluginName String plugin) {
    this.gApi = gApi;
    this.change = change;
